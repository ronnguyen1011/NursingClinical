...
<main class="container mt-3">
    <div class="row">
        ...
        if($_SESSION["Admin"]) {
            if($_GET["operation"] === "edit-requirements") { ?>
                <div class="col-12 col-md-8">
                    ...
                    if(isset($_POST["confirm-edits"]) && $_POST["confirm-edits"] === "confirmed") {
                        ...
                        foreach ($_POST as $postKey => $postValue) {
                            ...
                            if($postKey != "confirm-edits") {
                                ...
                                if($currRequirementID != $lastRequirementID) {
                                    ...
                                    $selectResult = executeQuery("SELECT * FROM ClinicalRequirements WHERE RequirementID = '{$currRequirementID}'");
                                ...
                                if($lastRequirement[$currColumnName] != $postValue) {
                                    $result = executeQuery("UPDATE ClinicalRequirements SET {$currColumnName} = '{$postValue}' WHERE RequirementID = '{$currRequirementID}'");
                        ...
                        echo generateMessageWithLink("/NursingClinical/view/requirements.php", "Clinical Requirements", "Changes were saved successfully");
                    } else {
                        echo generateMessageWithLink("/NursingClinical/Controller/admin/edit-requirements.php", "Edit Clinical Requirements", "Please edit Clinical Requirements there and try again", "ERROR: No changes received from Edit Requirements");
                    }
                    ?>
                </div>
                <?php
            ...
            elseif($_GET["operation"] === "delete-requirement") {
                ?>
                <div class="col-12 col-md-8">
                    <?php
                    if(isset($_POST["RequirementID"])) {
                        $requirementID = $_POST["RequirementID"];
                        $result = executeQuery("DELETE FROM ClinicalRequirements WHERE RequirementID = '{$requirementID}'");

                        if(!$result) {
                            echo generateMessage("Failed to delete requirement: {$result}",
                                "ERROR: Delete Failed");
                        } else {
                            // display success, and link to Edit Requirements page
                            echo generateMessageWithLink("edit-requirements.php", "Edit Requirements",
                                "Requirement deleted successfully");
                        }
                    } else {
                        echo generateMessage("Invalid submission", "ERROR: Delete Failed");
                    }
                    ?>
                </div>
                <?php
